...
- [~] **B4**: Activate Workspace for Nonprofits on `blackshearpta.org` - `JON` - **Explanation re-submitted 2026-09-24**, after the site actually went live ([A33](#phase-2---real-site)). Every claim in it is verifiable against `blackshearpta.org` as deployed: the group-exemption reason for the "Pta Texas Congress" name, EIN `91-2015074` listed as "612 Blackshear Elementary PTA" with the ProPublica link, and the homepage's *About our organization* section as the evidence the domain is ours. **Awaiting review.** Previously rejected 2026-09-15 ([F48](#f48)); the first attempt failed partly because the only page Google could reach disowned the domain, which is no longer true
