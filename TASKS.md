...
- [ ] **B4**: Activate Workspace for Nonprofits on `blackshearpta.org` - `JON` - **Next action: click Activate** on the GfN Workspace card - free, no payment method, ~3 business days to review. **Confirm first that the eligible domain is `blackshearpta.org`** (GfN → Settings); the card does not name it. ⚠️ **Do not complete the `workspace.google.com/checkout` flow** - that is paid Business Standard at $16.80/user/mo and is not this. See [F45](#f45)
