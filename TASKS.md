...
- [~] **B3**: Reconcile with whatever already exists on the domain - `JON` - **Largely resolved by answering the form correctly.** The [F8](#f8) conflict fired as *"This domain name is already in use"*, but that is an artifact of answering *No* to "do you currently use Google Workspace?" - answering **Yes** yields *"Your domain is eligible for an upgrade ... at no charge"* with an **Activate** button ([F45](#f45)). Not a DNS problem and possibly not a Gabe problem either. Gabe is only needed if the activation review rejects on ownership
- [!] **B4**: Activate Workspace for Nonprofits on `blackshearpta.org` - `JON` - **REJECTED 2026-09-15** - *"the connection between Pta Texas Congress and blackshearpta.org couldn't be verified."* Cause and remedy in [F47](#f47); the gate page has been fixed to carry the evidence. **Re-submit via the GfN support form once that fix is deployed - not before.** Previously: submitted 2026-09-12, confirmation email received. Nothing back as of 2026-09-14. Submission landed on a **Saturday**, so the "up to 3 business days" clock starts Mon 2026-09-14 and nominally expires **Thu 2026-09-17** - and the GfN products page is showing *"Product activation review wait times are longer than normal"*, so treat that date as soft. **No news before then carries no information; do not re-submit.** If it is rejected on ownership, that is the trigger for the Gabe fallback in [B3](#google-workspace-for-nonprofits). ⚠️ Still do not complete the `workspace.google.com/checkout` flow - paid Business Standard, $16.80/user/mo ([F45](#f45))
...
- [x] **A31**: **Remove the pre-launch gate** - `CLAUDE` + `JON` - **Done 2026-09-16.** `blackshearpta.org` now serves the real site to anyone, with no password and no construction notice. Driven by [B4](#google-workspace-for-nonprofits): Google could not verify the domain against the organization while the only reachable page was a construction notice pointing at Weebly ([F47](#f47)). Gate logic out of `src/worker.ts`, `src/pages/under-construction.astro` and `docs/PRE-LAUNCH-GATE.md` deleted, stale references cleaned out of six other files. **The Worker stays** - see [F48](#f48). Registration identity now renders on the homepage (`OrgIdentity.astro`) and in the site-wide footer. **`noindex` is deliberately still on** ([A29](#phase-2---real-site)). Jon can delete the now-unused `SITE_PASSWORD` secret from the Cloudflare dashboard at leisure
...
- [~] **A29**: **Cutover** - `JON` + `CLAUDE` - **Partly done.** The gate is gone ([A31](#phase-2---real-site)), so the site is publicly reachable. **Still open: retire Weebly, remove `noindex` in both places ([A6](#phase-0---accounts--scaffold)), flip [A22](#phase-2---real-site) to 301, submit a sitemap.** **Do not remove `noindex` before Weebly is actually retired** - two indexed copies of the same content is a ranking mess to unwind. ⚠️ This task's old instruction to *"delete `src/worker.ts` plus the three lines marked TEMPORARY in `wrangler.jsonc`"* was **wrong by the time it was acted on** - see [F48](#f48)
